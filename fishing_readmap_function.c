...
//* HOW TO PASS THE mxn DIMENSIONAL MAP ARRAY INTO THE GS STRUCT???
//
//  struct GS
//  {
//      player* players;
...
//  };
    ...
    struct GS GS1 = {players, map[1]};   // This passes only one row WHEN THE STRUCT IS DEFINED AS ABOVE.

    printf("\nTotal no of players: %d\n", GS1.players[0].player_no);
    printf("Name: %s\n", GS1.players[1].player_ID);
    ...
    //
    //     for (c = 0; c < columns; c++)
    //         printf("%d%d ", GS1->map[r][c].fish, GS1->map[r][c].penguin_owner);
    //
    //     printf("\n");
    // }
